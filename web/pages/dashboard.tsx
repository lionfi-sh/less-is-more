...
export default function Dashboard() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    (async () => {
      let { name, gpuType, imageUrl } = newJob;
      const auth_header = decodeURIComponent(
        document.cookie
          .split("; ")
      ...
      try {
        const { id, user_id, nickname, image_url, created_at } = (
          await axios.post(
        ...
        const image_versions = (
          await axios.get(`http://localhost:8080/images/${id}/versions`, {
            headers: { Authorization: auth_header },
          })
        ).data;
        const results = await axios.post(
          ...
          {
            image_id: id,
            image_version_id: image_versions[0].id,
            cpu: "performance",
            gpu: job.gpuType,
          },
          { headers: { Authorization: auth_header } },
      ...
      <div className="container mx-auto bg-white rounded-lg shadow-lg p-6">
        ...
        <Card className="mb-8">
          <CardHeader>
            ...
            <CardDescription>
              Fill in the details to create a new GPU job
            </CardDescription>
          </CardHeader>
          <CardContent>
                ...
                <div className="space-y-2">
                  ...
                  >
                    ...
                    <SelectContent>
                      ...
                      <SelectItem value="a100-40gb">A100</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              ...
              <TableBody>
                {jobs.map((job) => (
                  <TableRow key={job.id}>
                    ...
                    <TableCell>
                      <span
                        className={`inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ${job.status === "completed"
